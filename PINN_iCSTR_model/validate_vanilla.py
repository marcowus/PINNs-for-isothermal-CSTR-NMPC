...
MODEL_PATH = 'train_results_vanilla/vanilla_model.pth'

SAVE_DIR = 'val_results_vanilla'
...
def validate():
    ...
    print(f"Validating on {len(df_val)} points (rows {train_size} to {train_size+val_size-1})")

    model = Vanilla_NN()
    model.load_state_dict(torch.load(MODEL_PATH))
    model.eval()

    metrics, CA_true, CA_pred, t = rollout_prediction(model, df_val)

    # Plot
    plt.figure(figsize=(10,6))
    plt.plot(t, CA_true, label='True (Simulink)', lw=2)
    plt.plot(t, CA_pred, '--', label='Vanilla NN rollout', lw=2.5)
    plt.xlabel('Time')
    plt.ylabel('C_A concentration')
    plt.title(f'Validation Rollout (Vanilla NN)\nR² = {metrics["r2"]:.4f} | RMSE = {metrics["rmse"]:.6f} | MAE = {metrics["mae"]:.6f}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(SAVE_DIR, 'val_rollout.png'), dpi=200, bbox_inches='tight')
    plt.close()
